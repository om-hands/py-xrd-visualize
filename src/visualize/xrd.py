...
def fig_ω_scan_1axis(
    ...
    legend_title: str = "",
    legend_reverse: bool = False,
    optimize_func: util.Fitter = util.voigt(),
    ...
    for xy, p0 in zip(xys, p0s):
        ...
        xy.x -= center
        xy.y /= optimize_func.func(center, *popt)
    ...
    def fig_func_opt(fig: Figure):
        ...
        for popt, legend in zip(popts, legends):
            ...
            # center(x)=0
            y = np.vectorize(optimize_func.func)(x, popt[0], 0, *popt[2:])
            # y=1 on x=0
            y /= np.max(y)
            print(y)
        ...
        fig.suptitle("fit:{}".format(optimize_func.__class__))
